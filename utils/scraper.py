...
def _http_get(url: str, headers: dict | None = None) -> requests.Response:
    ...
    for _ in range(3):
        try:
            resp = requests.get(
                url, headers=hdrs, timeout=REQUEST_TIMEOUT, allow_redirects=True
            )
...
def scrape_single_article(url: str) -> dict:
    ...
    if not title:
        # RSSやその他のケースのフォールバック
        rss_title = soup.find("title")
        title = rss_title.get_text(strip=True) if rss_title else "No Title"
        title_tag = soup.find("h1")
        title = title_tag.get_text(strip=True) if title_tag else None
    if not title:
        og = soup.find("meta", attrs={"property": "og:title"})
    ...
    for p in soup.find_all("p", attrs={"data-testid": True}):
        txt = _cleanup_para(p.get_text(" ", strip=True))
        paragraphs.append(txt) if txt else None
    if not paragraphs:
