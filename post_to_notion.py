...
def _payload(title, url, published_at, tags, summary, background, key_points, actions):
    props = {
        ...
        "URL": {"url": url or None},
        "Published": {
            "date": {"start": (published_at or datetime.utcnow().isoformat() + "Z")}
        },
    }
    if tags:
        props["Tags"] = {"multi_select": [{"name": t} for t in tags if t]}
    children = []
    if summary:
        children += [
            {
    ...
    if key_points:
        children += [
            ...
            *[_blk(f"- {kp}") for kp in key_points],
        ]
    if actions:
        children += [
            {
...
def post_to_notion(
    title, url, published_at, tags, summary, background, key_points, actions
):
