...
if not NOTION_TOKEN or not DATABASE_ID:
    print(
        "❌ NOTION_TOKEN または DATABASE_ID が設定されていません。.envを確認してください。"
    )
    sys.exit(1)
...
def get_article_content(url):
    ...
    if pub_time_meta:
        utc_time = datetime.fromisoformat(
            pub_time_meta["content"].replace("Z", "+00:00")
        )
...
def post_to_notion(title, url, summary, pub_time, tags):
    ...
    res = requests.post(
        "https://api.notion.com/v1/pages", headers=headers, data=json.dumps(data)
    )
...
def main():
    ...
    rss_url = (
        f"https://www.bing.com/news/search?q={encoded_kw}+site:reuters.com&format=rss"
    )
